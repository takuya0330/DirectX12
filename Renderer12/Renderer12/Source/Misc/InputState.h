...
public:
	bool State(bool _IsPress, gl::Trigger _Trigger)
	{
		mPreviousState = mCurrentState;
		if (_IsPress) mCurrentState++;
		else mCurrentState = 0;

		if (_Trigger == gl::Trigger::ePress) return mPreviousState == 0 && mCurrentState > 0;
		else if (_Trigger == gl::Trigger::eRelease) return mPreviousState > 0 && mCurrentState == 0;
